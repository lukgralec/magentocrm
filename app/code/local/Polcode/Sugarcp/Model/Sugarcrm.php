...
////////////create new Product in SugarCRM/////////////////////////
    ...
    private function syncProduct($params) {
        ...
        $i = 0;
        $nameValueList = array();
        $collection = Mage::getModel('catalog/product')->getCollection()->addAttributeToSelect(array('name', 'sku', 'price'));
        ...
        foreach ($collection as $value) {
            ...
            $sku = $value['sku'];
            $price = $value['price'];
            $productFromMagento = array($name, $sku, $price);
            $id = Mage::helper('sugarcp')->getProductId($sku);
            ...
            if ($productFromMagento != $productFromSugar) {
                if ($id !== false) {
                    ...
                    $arr = array($i => array(
                            ...
                            array('name' => 'price', 'value' => $value['price']),
                        )
                    );
                    $nameValueList = array_merge($nameValueList, $arr);
                    $i++;
